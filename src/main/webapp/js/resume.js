...
/*字符拼接区*/
	var introduce='\/**\n * 嗨，我叫冮浩杨，ZJU CS大三在读\n * 首先必须说明，本页面参考自http://strml.net/，感谢开源的各位前辈 \n * 我只是个小菜鸡~~\n **\/\n';
	var trs1 = '\n\/* 首先,给所有元素加上过渡效果 *\/';
	var sty1 = '\n * {\n  transition: all .3s;\n }\n';
	var trs2 = '\n\/* 然后换个背景 *\/';
	var sty2 = '\n html {\n  background: rgb(177,255,47);\n }\n';
	var trs3 = '\n\/* 让我们加个边框 *\/';
 ...
 //    var trs3 = '\n  \/*对了，这个简历是markdown语法，应该改成html才看着舒服。\n  *接下来变个魔术\n  *倒数3个数字\n  *3......\n  *2......\n  *1......\n  *OK,这就是为您准备的菜，祝享用愉快！ */'
	...
	var str = introduce.concat(trs1).concat(sty1).concat(trs2).concat(sty2).concat(trs3).concat(sty3).concat(trs4).concat(lightheight).concat(trs5).concat(scrollRight).concat(drawBoard);
	/*常规定义区*/
...
var controller= setInterval(put,10);
	 function put(){
		...
		/*创建pre简历板*/
			if(n>=672&&n<=929){
				...
				if(document.getElementById('drawBoard')){
					...
					console.log("yes!!!!!!!!");
					var drawBoard = document.createElement('img');
				    drawBoard.id = 'drawBoard';

				    document.body.appendChild(drawBoard);
					drawBoard.src="./img/CV.jpg";
	    ...
	    //  }
		 ...
		 if(n>3000){window.clearInterval(controller);};
